...
#        node_view = importlib.import_module("nodeviews.%s.%s" %(node_class,method))
        ...
        if type(graphics_data) == ListVector:
            ...
            self.graphics_data[0]['href'] = context.request.static_url('dotpipeRwsgi:'+urljoin('static/tmp/', os.path.basename(graphics_elmt.rx2(1)[0]))) # FIXME: this could break
...
class Digraph(object):
    ...
    def draw_svg(self,request):
        ...
        return(request.static_path('dotpipeRwsgi:'+os.path.join('static','tmp',os.path.split(path)[1])))
