...
from .models import Digraph, NodeData
...
class NonPipelineViews(App):
    ...
    def browse(self):
        if self.user:
            mypipelines = DBSession.query(Pipeline).filter(Pipeline.owner==self.user.uid).order_by(Pipeline.id).all()
        else:
            mypipelines = [] 
#        for pipeline in mypipelines:
#            pipeline.load_dot(self.request)
        public_pipelines = DBSession.query(Pipeline).filter(Pipeline.permissions%2==1).order_by(Pipeline.id).all()
        print("list_pipelines:")
        print(asbool(self.request.registry.settings['list_pipelines']))
        return dict(title='Pipelines', mypipelines=mypipelines, public_pipelines=public_pipelines, list_pipelines=asbool(self.request.registry.settings['list_pipelines']))
    ...
    def create(self):
        ...
        print(self.request.registry.settings['uploadpath'])
...
#        pipeline.load_dot(self.request) #FIXME: we are doing some extra lookup here
        pipeline.dot = Digraph.load_from_file_path(file_path)
#        pipeline.dot = Dot.load_from_file_path(file_path) #NOTE: whenever we use this, we have to set the dot's pipeline manually
    ...
    def files(self):
        ...
        try:
            path = os.path.join(self.request.registry.settings['uploadpath'],self.request.context.user.username)
        ...
        for name, fieldStorage in self.request.POST.items():
            ...
            if self.validate(result):
                ...
                if re.match(filename_pattern, result['name']) is None:
                    ...
                    if not os.path.exists(self.request.registry.settings['uploadpath']):
                        os.makedirs(self.request.registry.settings['uploadpath'])
                    d = os.path.join(self.request.registry.settings['uploadpath'],self.request.context.user.username)
...
#            file_path = os.path.join(os.path.join(self.request.registry.settings['uploadpath'],self.logged_in),filename)
#            input_file = self.request.POST['file'].file
...
#        return HTTPFound(location = self.request.route_url('view', pipeline_id=self.pipeline.id))
    ...
    @view_config(route_name='view_source',
                 permission='view',
                 renderer='templates/view_source.pt')
    def view_source(self):
        ...
        code = self.pipeline.dot.source()
        source = highlight(code, lexer, formatter)
        return dict(pipeline=self.pipeline, title=self.pipeline.name, request=self.request, source=source, css=css)
...
#        call = self.request.matchdict['node_call']
        ...
        if self.request.session['tmpdot'] is None:
            raise NotFound
        dot = Digraph.load_from_file_path(self.request.session['tmpdot'])
        if dot is None:
            raise NotFound
        pipe_to = []
