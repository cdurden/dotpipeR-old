...
subgraph_preamble : /* empty */ | SUBGRAPH ID /* This rule and the next change the graph pointer so that subgraphs are ignored. If we want to implement reading subgraphs, we could use graphs of graphs, setting the graph pointer to a graph created as a child node, and setting the pointer back to the parent node when done */

subgraph_begin : subgraph_preamble
{
...
attr_stmt : NODE attr_list | EDGE attr_list;
...
{
  list_insert(((GraphDataInterface *)gdi)->preprocessor_lines, list_tail(((GraphDataInterface *)gdi)->preprocessor_lines), $2);
};
