...
from .exceptions import DotReadError, PipelineEditConflict, RViewRuntimeError, RPipelineNodeCacheError, RPipelineReadError
...
#        node_view = importlib.import_module("nodeviews.%s.%s" %(node_class,method))
        ...
        print("running R view code")
        try:
            graphics_data = r('view(pipeline["%s"],tmpdir="%s")' %(self.node,tmpdir))
        except RRuntimeError as e:
            graphics_data = None
            pass #FIXME: should we return some error mesage to the user in this case?
#            raise RViewRuntimeError(node_class,method,e)
        self.graphics_data = []
        if type(graphics_data) == ListVector:
            for i in range(0,len(graphics_data)): #FIXME: we are counting on R code to return a well-formatted object
                ...
                self.graphics_data[i]['href'] = context.request.static_url('tutorial:'+urljoin('static/tmp/',os.path.basename(self.graphics_data[i]['file']))) # FIXME: this could break
    ...
    def eval_in_R(self, context):
        if self.dot.file_path is None:
            ...
            try:
                ...
                r('pipeline <- eval.PipelineNodes(pipeline,"%s",cacheMethod=cacheMethodPipelineDigest,cachedir="%s",do.eval=FALSE,envir=envir)' %(self.node,settings.dotpipeR_cachedir))
            except RRuntimeError as e:
...
#            r('pipeline <- eval.PipelineNode(pipeline,"%s",cacheMethod=cacheMethodPipelineDigest,do.eval=FALSE)' %node)
    ...
    def draw_svg(self,request):
        ...
        return(request.static_url('tutorial:'+os.path.join('static','tmp',os.path.split(path)[1])))
...
#            fns.extend(list(r('ls(envir=envir)'))) #FIXME: we should be running everything in a clean environment and then we can ls this environment
    ...
    def load_dot(self, request):
        try:
            self.dot = Dot.load_from_url(self.url(request))
        except IOError as e:
            raise(DotReadError(e.msg))
        except TypeError as e:
            raise(DotReadError(e)) #FIXME: need to handle general exception
        ...
        if os.fork() == 0:
            try:
                ...
                try:
                    ...
                    r('envir <- as.environment(which(search()=="%s")[1])' %settings.dotpipeR_env_name)
                    r('pipeline <- exec.Pipeline(pipeline,cachedir="%s",envir=envir)' %settings.dotpipeR_cachedir)
                    r('linkCaches.Pipeline(pipeline,cacheMethodSource=cacheMethodDigest,cacheMethodDest=cacheMethodPipelineDigest,cachedir="%s",overwrite=TRUE,envir=envir)' %settings.dotpipeR_cachedir)
                except EnvironmentError as e:
